...
GamePawn::GamePawn(glm::vec3 colorID, GLfloat alivetime, GLuint score, glm::vec2 pos, glm::vec2 size, Texture2D sprite, GLboolean isspecial, specialpawn type, glm::vec3 color, GLfloat hue, GLfloat rotation) : ColorID(colorID), aliveTime(alivetime), Score(score), Position(pos), Size(size), Color(color), Rotation(rotation), Sprite(sprite), isDestroyed(GL_FALSE), special(isspecial), specialtype(type), Hue(hue)
{
